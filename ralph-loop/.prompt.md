...
Current task: TASK-010 - Add deterministic seed data, reset flow, and end-to-end test hardening
Target output file: ralph-loop/results/10-seeds-and-tests.md

Hard requirements:
- Implement the task directly in the repository.
- Read the PRD and check your work against the referenced PRD section.
- Follow AGENTS.md and docs/ constraints.
- Preserve shadcn/ui usage for frontend work.
- End with STATUS: DONE or STATUS: BLOCKED.

Task description:
Create the seeded validation environment described in the PRD and strengthen backend, frontend, and E2E tests around the new navigation and onboarding flow.

PRD section to satisfy:
Story 7: Validate and test against realistic seeded data; REQ-012

Acceptance target:
Two collaborations with three studies each can be seeded through a documented reset flow, and the main UX journeys are covered by container-first tests.

Task notes:
Follow AGENTS.md Docker-first rules and docs/05-DEVOPS_AND_TESTING.md. Prefer reproducible factories and deterministic seeds.
